...
function showDropdownResults() {
    ...
    matchedItems.forEach(item => {
        ...
        resultDiv.innerHTML = `
            <span>${item.name}</span>
            <button class="basket-button" onclick="addToBasket('${item.name}')">Add to Basket</button>
        `;
        resultDiv.onclick = function(event) {
            // Prevent clicking on the basket button from redirecting
...
function addToBasket(itemName) {
    // Implement the logic to add the item to the basket here
    console.log(`Adding ${itemName} to basket.`);
